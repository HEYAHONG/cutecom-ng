    ...
    if(check->count > 0)
    {
        for(size_t i=0; i<files->count; i++)
        {
            if(strcmp("-",files->basename[i])==0)
            {
                while(!feof(stdin))
                {
                    char fileline[sizeof(hsha3_sha512_t)+4096]= {0};
                    if(NULL!=fgets(fileline,sizeof(fileline)-1,stdin))
                    {
                        ...
                        hsha3_sha512_t sha3_sha512_hash= {0};
                        hbase16_decode(sha3_sha512_hash,sizeof(sha3_sha512_hash),hash_string,strlen(hash_string));
                        hsha3_sha512_t sha3_sha512_file= {0};
                        {
                            hsha3_sha512_context_t ctx;
                            hsha3_sha512_starts(&ctx);
                            std::fstream file;
                            ...
                            while(!file.eof())
                            {
                                uint8_t buffer[sizeof(hsha3_sha512_message_block_t)];
                                file.read((char *)buffer,sizeof(buffer));
                                hsha3_sha512_update(&ctx,buffer,file.gcount());
                            }
                            file.close();
                            hsha3_sha512_finish(&ctx,sha3_sha512_file);
                        }

                        if(memcmp(sha3_sha512_file,sha3_sha512_hash,sizeof(hsha3_sha512_t))==0)
                        {
            ...
            for(size_t i=0; i<files->count; i++)
            {
                if(strcmp("-",files->basename[i])==0)
                {
                    ...
                    while((c=getchar())!=EOF)
                    {
                        ...
                        hsha3_sha512_update(&ctx,&ch,sizeof(ch));
                    }
                    hsha3_sha512_t sha3_sha512= {0};
                    hsha3_sha512_finish(&ctx,sha3_sha512);
                    char output_string[sizeof(sha3_sha512)*2+1]= {0};
                    hbase16_encode_with_null_terminator(output_string,sizeof(output_string),sha3_sha512,sizeof(sha3_sha512));
                    hfprintf(stdout,"%s -\r\n",output_string);
                }
                else
                {
