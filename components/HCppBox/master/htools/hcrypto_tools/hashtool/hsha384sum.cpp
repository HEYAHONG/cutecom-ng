    ...
    if(check->count > 0)
    {
        for(size_t i=0; i<files->count; i++)
        {
            if(strcmp("-",files->basename[i])==0)
            {
                while(!feof(stdin))
                {
                    char fileline[sizeof(hsha2_sha384_t)+4096]= {0};
                    if(NULL!=fgets(fileline,sizeof(fileline)-1,stdin))
                    {
                        ...
                        hsha2_sha384_t sha2_sha384_hash= {0};
                        hbase16_decode(sha2_sha384_hash,sizeof(sha2_sha384_hash),hash_string,strlen(hash_string));
                        hsha2_sha384_t sha2_sha384_file= {0};
                        {
                            ...
                            while(!file.eof())
                            {
                                ...
                                hsha2_sha384_update(&ctx,buffer,file.gcount());
                            }
                            file.close();
                            hsha2_sha384_finish(&ctx,sha2_sha384_file);
                        }

                        if(memcmp(sha2_sha384_file,sha2_sha384_hash,sizeof(hsha2_sha384_t))==0)
                        {
            ...
            for(size_t i=0; i<files->count; i++)
            {
                if(strcmp("-",files->basename[i])==0)
                {
                    ...
                    while((c=getchar())!=EOF)
                    {
                        ...
                        hsha2_sha384_update(&ctx,&ch,sizeof(ch));
                    }
                    hsha2_sha384_t sha2_sha384= {0};
                    hsha2_sha384_finish(&ctx,sha2_sha384);
                    char output_string[sizeof(sha2_sha384)*2+1]= {0};
                    hbase16_encode_with_null_terminator(output_string,sizeof(output_string),sha2_sha384,sizeof(sha2_sha384));
                    hfprintf(stdout,"%s -\r\n",output_string);
                }
                else
                {
