    ...
    if(check->count > 0)
    {
        for(size_t i=0; i<files->count; i++)
        {
            if(strcmp("-",files->basename[i])==0)
            {
                while(!feof(stdin))
                {
                    char fileline[sizeof(hmd5_md5_t)+4096]= {0};
                    if(NULL!=fgets(fileline,sizeof(fileline)-1,stdin))
                    {
                        ...
                        hmd5_md5_t md5_hash= {0};
                        hbase16_decode(md5_hash,sizeof(md5_hash),hash_string,strlen(hash_string));
                        hmd5_md5_t md5_file= {0};
                        {
                            hmd5_context_t ctx;
                            hmd5_starts(&ctx);
                            std::fstream file;
                            ...
                            while(!file.eof())
                            {
                                uint8_t buffer[sizeof(hmd5_message_block_t)];
                                file.read((char *)buffer,sizeof(buffer));
                                hmd5_update(&ctx,buffer,file.gcount());
                            }
                            file.close();
                            hmd5_finish(&ctx,md5_file);
                        }

                        if(memcmp(md5_file,md5_hash,sizeof(hmd5_md5_t))==0)
                        {
            ...
            for(size_t i=0; i<files->count; i++)
            {
                if(strcmp("-",files->basename[i])==0)
                {
                    ...
                    while((c=getchar())!=EOF)
                    {
                        ...
                        hmd5_update(&ctx,&ch,sizeof(ch));
                    }
                    hmd5_md5_t md5= {0};
                    hmd5_finish(&ctx,md5);
                    char output_string[sizeof(md5)*2+1]= {0};
                    hbase16_encode_with_null_terminator(output_string,sizeof(output_string),md5,sizeof(md5));
                    hfprintf(stdout,"%s -\r\n",output_string);
                }
                else
                {
