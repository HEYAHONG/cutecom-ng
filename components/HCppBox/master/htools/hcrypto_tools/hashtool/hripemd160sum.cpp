    ...
    if(check->count > 0)
    {
        for(size_t i=0; i<files->count; i++)
        {
            if(strcmp("-",files->basename[i])==0)
            {
                while(!feof(stdin))
                {
                    char fileline[sizeof(hripemd160_ripemd160_t)+4096]= {0};
                    if(NULL!=fgets(fileline,sizeof(fileline)-1,stdin))
                    {
                        ...
                        hripemd160_ripemd160_t ripemd160_hash= {0};
                        hbase16_decode(ripemd160_hash,sizeof(ripemd160_hash),hash_string,strlen(hash_string));
                        hripemd160_ripemd160_t ripemd160_file= {0};
                        {
                            hripemd160_context_t ctx;
                            hripemd160_starts(&ctx);
                            std::fstream file;
                            ...
                            while(!file.eof())
                            {
                                uint8_t buffer[sizeof(hripemd160_message_block_t)];
                                file.read((char *)buffer,sizeof(buffer));
                                hripemd160_update(&ctx,buffer,file.gcount());
                            }
                            file.close();
                            hripemd160_finish(&ctx,ripemd160_file);
                        }

                        if(memcmp(ripemd160_file,ripemd160_hash,sizeof(hripemd160_ripemd160_t))==0)
                        {
            ...
            for(size_t i=0; i<files->count; i++)
            {
                if(strcmp("-",files->basename[i])==0)
                {
                    ...
                    while((c=getchar())!=EOF)
                    {
                        ...
                        hripemd160_update(&ctx,&ch,sizeof(ch));
                    }
                    hripemd160_ripemd160_t ripemd160= {0};
                    hripemd160_finish(&ctx,ripemd160);
                    char output_string[sizeof(ripemd160)*2+1]= {0};
                    hbase16_encode_with_null_terminator(output_string,sizeof(output_string),ripemd160,sizeof(ripemd160));
                    hfprintf(stdout,"%s -\r\n",output_string);
                }
                else
                {
